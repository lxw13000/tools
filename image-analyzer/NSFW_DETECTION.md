...
请求进入
  ...
  │    └─ 超时 → 503 "服务繁忙"
  │
  ├─ URL 安全校验（协议白名单 + 私有 IP 拦截）
  │    └─ 不合法 → 下载返回 None → 400
  │
  ├─ 下载图片（连接超时+读取超时，限 50MB，流式分块写入）
  │    └─ 失败 → 400 "图片下载失败"
  │
  ├─ 图片格式验证（PIL verify）
  │    └─ 非图片 → 400
...
| URL 安全校验 | 协议 + IP | 仅允许 http/https，拦截私有/保留 IP（防止 SSRF） |
| 双超时下载 | `download_timeout: 15` | 连接超时 15s + 读取超时 30s，防止线程无限阻塞 |
| 文件大小限制 | `max_file_size: 50MB` | 流式分块检查，超限立即中止并删除 |
| 图片格式验证 | PIL verify | 下载后校验文件是否为有效图片，防止非图片文件进入模型 |
| 线程安全推理 | threading.Lock | 模型推理 Lock 串行化，避免多线程并发 crash |
| 模型熔断保护 | `_load_failed` | 模型加载失败后标记熔断，不再重试消耗资源 |
| 全链路异常隔离 | try/except 每环节 | 下载、检测、清理各自独立捕获，互不影响 |
| 临时文件强制清理 | finally 块 | 无论成功失败，始终删除临时文件 |
| 请求 ID 追踪 | `request_id` | 12 位 UUID，贯穿全部日志输出，便于排查 |
| 日志轮转 | RotatingFileHandler | 50MB/文件 x 5 个历史，防止磁盘占满 |

### 8.3 模型懒加载

三个模型均采用懒加载策略：首次调用时才初始化，避免启动时一次性加载全部模型占用过多内存。加载后常驻内存，后续调用无需重复加载。加载过程使用双重检查锁（double-check locking）保证线程安全。
